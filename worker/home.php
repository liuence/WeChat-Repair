...
require_once("../db.php");
$sql3 = "SELECT * FROM `worker`  where openid='".$openid."'";
...
$worker1=$arr3[0]['name'];
setcookie("name", $worker1);
if(empty($arr3))
{
    
echo "<script type='text/javascript'> alert('对不起，您没有权限！');location.href='index.php';</script>";

}else{
echo "<script type='text/javascript'> alert('登录成功！');location.href='home.php';</script>";	
}
}
...
<?php
    ...
    require_once("../db.php");  
    $sql = "SELECT * FROM `info` where worker like '%$worker%' order by `id` desc limit $page $pagesize ";
...
<div data-role="page" id="page1" data-ajax="false" data-fullscreen="false">
    ...
    <div data-theme="a" data-role="header" data-position='fixed' data-tap-toggle="false">
        ...
        <a href="#rightpanel" data-role="button" data-inline="true" data-icon="bars" data-ajax="false"  data-iconpos="notext"> Default panel</a>   
    </div>
...
$sql1 = "SELECT * FROM `info` where worker like '%$worker%' order by `id` desc ";
