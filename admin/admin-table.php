<?php
if (!empty($_POST['sub'])){       
   setcookie("region", $_POST['region'],time()+3600);	
...
<option value="所有单元">所有单元</option>
<?php if( is_array( $arr3 ) ): ?>
<?php foreach(  $arr3 as $item3 ): ?>  
                <option value="<?php echo $item3['region'];?>" <?php if($item3['region']==$_COOKIE["region"]){echo 'selected="selected"';}?>>
                    <?php echo $item3['region'];?>
                </option>
<?php endforeach; ?>
<?php endif; ?>				
</select>  
...
<?php foreach(  $arr5 as $item5 ): ?>  			
                <option value="<?php echo $item5['status'];?>" <?php if($item5['status']==$_COOKIE["status"]){echo 'selected="selected"';}?>>
...
<?php
   ...
   if($_COOKIE["region"]=="所有单元"&&$_COOKIE["status"]=="全部"){
   $sql = "SELECT * FROM `info` order by `id` desc "; 
   }
   if($_COOKIE["region"]=="所有单元"&&$_COOKIE["status"]!=="全部"&&$_COOKIE["status"]!="")
   {
   $sql = "SELECT * FROM `info` where status='".$_COOKIE["status"]."' order by `id` desc "; 
   }
   if($_COOKIE["region"]!=="所有单元"&&$_COOKIE["region"]!=""&&$_COOKIE["status"]=="全部")
   {
...
?>    
<div align="right">
    <input name="sub" type="submit" class="buttom"  onClick="location.href='csv-info.php?region=<?php echo $_COOKIE["region"]; ?>&status=<?php echo $_COOKIE["s"]; ?>'" value="导出表单"/>
      </div>
