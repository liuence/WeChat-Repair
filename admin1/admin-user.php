...
echo "<script language=javascript>window.window.location.href='member-result.php?keyword=$keyword ';</script>";
}
   $role=$_COOKIE["role"];
   $pagesize=20;
	if($_GET[page]){
   ...
   require_once("../db.php");  
   $sql2 = "SELECT * FROM `user` where region like '%$role%' order by `id` desc limit $page $pagesize ";
